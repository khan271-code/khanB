...
def create_spectrogram_strip_each_second(signals, fs, lead_names, output_dir, total_seconds=10, gap_px=5):
    ...
    for sec in range(total_seconds):
        ...
        h, w = combined.shape
        max_side = max(h, w)
        square_image = np.zeros((max_side, max_side))  # 黑色背景

        # 居中放置原图
        start_h = (max_side - h) // 2
        start_w = (max_side - w) // 2
        square_image[start_h:start_h+h, start_w:start_w+w] = combined

        plt.figure(figsize=(6, 6))
        plt.imshow(square_image, aspect='auto', origin='lower', cmap='viridis')
        plt.axis('off')
        plt.title(f"12-Lead Spectrogram {t_start:.0f}-{t_end:.0f}s", fontsize=12)

        out_path = os.path.join(output_dir, f'spectrogram_stripblack_{sec}_{sec+1}s.png')
        plt.savefig(out_path, bbox_inches='tight', pad_inches=0, dpi=200)
        plt.close()
        print(f"🖼️ Saved (square): {out_path}")
