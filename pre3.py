...
def create_spectrogram_strip_each_second_square(signals, fs, lead_names, output_dir, total_seconds=10, gap_px=5):
    """
    每秒生成一张12导联的正方形拼接图（上6下6，有黑色间隔）
    """
    ...
    for sec in range(total_seconds):
        ...
        # 统一高度
        min_height = min(img.shape[0] for img in lead_images)
        lead_images = [img[:min_height, :] for img in lead_images]

        # 黑色间隔
        gap_v = np.zeros((min_height, gap_px))  # 垂直
        gap_h = np.zeros((gap_px, (lead_images[0].shape[1] + gap_px) * 6 - gap_px))  # 水平

        # 上 6 个导联
        row_top = lead_images[0]
        for img in lead_images[1:6]:
            row_top = np.concatenate((row_top, gap_v, img), axis=1)

        # 下 6 个导联
        row_bottom = lead_images[6]
        for img in lead_images[7:12]:
            row_bottom = np.concatenate((row_bottom, gap_v, img), axis=1)

        # 拼成正方形
        combined = np.concatenate((row_top, gap_h, row_bottom), axis=0)
        ...
        plt.imshow(square, aspect='auto', origin='lower', cmap='viridis')
        plt.axis('off')
...
if __name__ == "__main__":
    ...
    # 5. 生成正方形拼接图
    create_spectrogram_strip_each_second_square(filtered_signals, fs, lead_names, output_dir, total_seconds=10, gap_px=5)
